...
router.put('/update_wish/', async function(req, res, next) {
  ...
  var id = req.body.id;
  var d = new Date();
  const url_pos = 'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input='+citta+'&inputtype=textquery&fields=photos,geometry&key='+process.env.API;
  ...
  serv.updateWish(id, rev, obj).then(({data, headers, status}) => {
...
router.post("/add_wish/", async function(req, res){
  ...
  serv.creaWish(obj).then(({data, headers, status}) => {
      console.log("");
